...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        cornerCheck = [(pacmanPosition == self.corners[i]) for i in range(4)]
        self.startState = (pacmanPosition, (cornerCheck[0], cornerCheck[1], cornerCheck[2], cornerCheck[3]))
        # (x, y), (top_left, top_right, down_left, down_right)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        position, cornerCheck = state
        for corner in cornerCheck:
            if not corner:
                return False
        return True
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                nextCornerCheck = [(True if nextPosition == self.corners[i] else cornerCheck[i]) for i in range(4)]

                nextState = (nextPosition, (nextCornerCheck[0], nextCornerCheck[1], nextCornerCheck[2], nextCornerCheck[3]))

                cost = 0
                for i in range(4):
                    if nextCornerCheck[i]:
                        cost += 1
...
def cornersHeuristic(state, problem):
    ...
    pos, cornerCheck = state
    val = 0
    count = 0
    for i in range(4):
        if not cornerCheck[i]:
            val = max(val, manhattan(pos, corners[i]))
            # val = min(val, euclidean(pos,corners[i]))
            count += 1
